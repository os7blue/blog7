...
<head>
    <meta charset="UTF-8">
    <title>${(errorCode??)?string("${errorCode}","")}-出错了！！！</title>
...
<div class="error">
    <div class="container-floud">
        <div style="text-align: center">
            ...
            <a href="/"><h2 class="h1">哎呦，出错了，点这里返回首页</h2></a>
        </div>
    </div>
</div>
...
<script>
    ...
    loop3 = setInterval(function () {
        if (i > 40) {
            clearInterval(loop3);
            document.querySelector('.thirdDigit').textContent = ${(errorCode??)?string("${errorCode[0]}","")};
        } else {
    ...
    loop2 = setInterval(function () {
        if (i > 80) {
            clearInterval(loop2);
            document.querySelector('.secondDigit').textContent = ${(errorCode??)?string("${errorCode[1]}","")};
        } else {
    ...
    loop1 = setInterval(function () {
        if (i > 100) {
            clearInterval(loop1);
            document.querySelector('.firstDigit').textContent = ${(errorCode??)?string("${errorCode[2]}","")};
        } else {
